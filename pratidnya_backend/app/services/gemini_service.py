...
class GeminiService:
    ...
    async def generate_structured_case_analysis(
        ...
        custody_status = facts_payload.get('custody_status', 'JUDICIAL_CUSTODY')
        arrest_date = facts_payload.get('arrest_date')
        days_in_custody = facts_payload.get('days_in_custody')
        if days_in_custody is not None:
            custody_detail = f"{custody_status} (सत्यापित अवधि: {days_in_custody} दिन)"
        elif arrest_date:
            custody_detail = f"{custody_status} (गिरफ्तारी दिनांक: {arrest_date})"
        else:
            custody_detail = f"{custody_status} (विचारण के दौरान निरुद्ध)"
        ...
        - अभियुक्त की स्थिति एवं अभिरक्षा: {custody_detail}
        ...
        3. घटना स्थल, समय एवं प्रक्रियात्मक त्रुटियों को रक्षा के तथ्यों से जोड़ें।
        ...
        6. न्यायिक अभिरक्षा की अवधि का विधिक नियम: अभिरक्षा की अवधि को केवल सत्यापित तथ्यों ({custody_detail}) के अनुसार ही उद्धृत करें। कभी भी मनगढ़ंत या काल्पनिक दिनों/महीनों का उल्लेख न करें।
        7. धारा 480 BNSS एवं निर्दोषिता के दावों का कठोर विधिक पृथक्करण:
           धारा 480 BNSS (समतुल्य 437 CrPC) केवल गैर-जमानती मामलों में मजिस्ट्रेट न्यायालय द्वारा जमानत दिए जाने की प्रक्रियात्मक/सांविधिक अधिकारिता (Procedural Power) है।
           निर्दोषिता के दावों (Claims of Innocence) में धारा 480 BNSS का प्रयोग सर्वथा अनुचित एवं त्रुटिपूर्ण है।
           निर्दोषिता के दावों को केवल और केवल (क) संविधान के अनुच्छेद 21, (ख) दांडिक न्यायशास्त्र की निर्दोषिता की उपधारणा (Presumption of Innocence until proven guilty), (ग) आपराधिक मंशा (Mens Rea) व सांविधिक तत्वों का अभाव, एवं (घ) दुर्भावनापूर्ण मिथ्या नामजदगी के आधार पर ही प्रस्तुत करें। धारा 480 BNSS को केवल आवेदन की प्रक्रियात्मक अधिकारिता के रूप में रखें।
        8. कठोर तथ्यात्मक सुसंगतता (Strict Factual Consistency): यदि मामले में घटना स्थल पर चेकिंग, नाकाबंदी अथवा मौके पर ही गिरफ्तारी (Spot Arrest / Checking) दर्शित है, तो अभियोजन की कमजोरी (prosecution_weaknesses) में कभी भी 'एफ.आई.आर. में अकारण विलंब' का बिंदु न जोड़ें। ऐसे मामलों में केवल स्वतंत्र साक्षियों का अभाव, जीडी एंट्री की विधिक त्रुटि, और बरामदगी मेमो सीलिंग में प्रक्रियात्मक खामी को ही अभियोजन की कमजोरी बताएं।
        ...
        {{
          ...
          "prosecution_weaknesses": [
            ...
            "अभियोजन कथानक में गंभीर विधिक विरोधाभास एवं साक्षियों के बयानों में अपूरणीय विसंगति होना।"
          ],
        ...
        factual_text = (str(facts_payload.get("factual_summary", "")) + " " + str(facts_payload.get("custody_status", ""))).lower()
        is_spot_arrest = any(kw in factual_text for kw in ["मौके", "चेकिंग", "तलाशी", "नाकाबंदी", "गिरफ्तारी", "spot", "checking", "patrol"])
        ...
        ] if is_spot_arrest else [
            "कथित घटना एवं बरामदगी के समय किसी भी स्वतंत्र व निष्पक्ष लोक साक्षी का उपस्थित न होना।",
            "अभियोजन कथानक में गंभीर विरोधाभास एवं साक्षियों के बयानों में अपूरणीय विसंगति होना।"
        ]

        normalized_weaknesses = _normalize_to_list(
            raw_draft.get("prosecution_weaknesses"),
            default_weaknesses
        )

        if is_spot_arrest:
            # Strictly eliminate hallucinated FIR delay on spot arrest
            filtered_weaknesses = []
            for w in normalized_weaknesses:
                if any(delay_kw in w for delay_kw in ["एफ.आई.आर.", "प्राथमिकी", "विलंब", "देरी"]):
                    continue
                filtered_weaknesses.append(w)
            if not filtered_weaknesses:
                filtered_weaknesses = default_weaknesses
            raw_draft["prosecution_weaknesses"] = filtered_weaknesses
        else:
            raw_draft["prosecution_weaknesses"] = normalized_weaknesses
